...
class ext_rest_dropboxpidgin_formatter extends default_formatter {
	...
	public function getDetailViewFormat() {
		...
		$yahoo_id_field_name = !empty($mapping['beans'][$this->_module]['yahoo_id']) ? $mapping['beans'][$this->_module]['yahoo_id'] : '';
		if(empty($yahoo_id_field_name)) {
			$GLOBALS['log']->error('DropboxPidgin Connector: '.$GLOBALS['app_strings']['ERR_MISSING_MAPPING_ENTRY_FORM_MODULE']);
			return '';
		}

		$this->_ss->assign('module', $_REQUEST['module']);
		$this->_ss->assign('record', $_REQUEST['record']);
		$this->_ss->assign('yahoo_id_field_name', $yahoo_id_field_name);
